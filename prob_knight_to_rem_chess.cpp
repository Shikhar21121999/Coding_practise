...
float recur(int i,int j,int step_lef){
    ...
    // recurrent case
    float probab=0;
    probab+=0.125*recur(i-2,j-1,step_lef-1);
    probab+=0.125*recur(i-2,j+1,step_lef-1);
    probab+=0.125*recur(i+2,j-1,step_lef-1);
    probab+=0.125*recur(i+2,j+1,step_lef-1);
    probab+=0.125*recur(i-1,j-2,step_lef-1);
    probab+=0.125*recur(i-1,j+2,step_lef-1);
    probab+=0.125*recur(i+1,j-2,step_lef-1);
    probab+=0.125*recur(i+1,j+2,step_lef-1);
...
#endif
    // input format
    // first line contains a single integer order of the chess matrix
    // second line contains two integer i,j the initial co-ordinates of knight piece
